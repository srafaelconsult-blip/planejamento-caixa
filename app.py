...
class PlanejamentoCaixa:
    def __init__(self, num_meses=5):
        ...
        self.fornecedores_anteriores = [0] * num_meses # NOVO CAMPO
    ...
    def calcular(self, dados):
        ...
        self.fornecedores_anteriores = [float(v) for v in dados.get('fornecedores_anteriores', [0]*self.num_meses)] # NOVO CAMPO
        ...
        for mes, venda in enumerate(self.vendas_escalonadas):
            valor_a_parcelar = venda - self.vendas_vista[mes]
            valor_parcela = valor_a_parcelar / n_parcelas_vendas if n_parcelas_vendas > 0 else 0
            for p_idx in range(n_parcelas_vendas):
                # **CORREÇÃO**: Iniciar no mês seguinte (mes + p_idx + 1)
                mes_recebimento = mes + p_idx + 1
                if mes_recebimento < self.num_meses:
                    self.duplicatas_receber[p_idx][mes_recebimento] += valor_parcela

        # 2. FLUXO DE COMISSÕES
        self.comissoes_mes = [v * float(self.setup.get('comissoes', 0)) for v in self.vendas_escalonadas]
        self.comissoes_pagas_vista = [c * 0.3 for c in self.comissoes_mes] # Assumindo 30% à vista
        ...
        for mes, comissao in enumerate(self.comissoes_mes):
            valor_a_parcelar = comissao * 0.7 # 70% parcelado
            valor_parcela = valor_a_parcelar / n_parcelas_comissoes if n_parcelas_comissoes > 0 else 0
            for p_idx in range(n_parcelas_comissoes):
                # **CORREÇÃO**: Iniciar no mês seguinte (mes + p_idx + 1)
                mes_pagamento = mes + p_idx + 1
                if mes_pagamento < self.num_meses:
                    self.comissoes_pagar_parcelado[p_idx][mes_pagamento] += valor_parcela
        
        comissoes_parceladas_mes = [sum(p[m] for p in self.comissoes_pagar_parcelado) for m in range(self.num_meses)]
        
        # **CORREÇÃO**: Soma total das comissões
        self.total_comissoes = [
            self.comissoes_pagas_vista[m] + comissoes_parceladas_mes[m] + self.comissoes_anteriores[m]
        ...
        for mes, compra in enumerate(self.compras_planejadas):
            valor_a_parcelar = compra - self.compras_vista[mes]
            valor_parcela = valor_a_parcelar / n_parcelas_compras if n_parcelas_compras > 0 else 0
            for p_idx in range(n_parcelas_compras):
                # **CORREÇÃO**: Iniciar no mês seguinte (mes + p_idx + 1)
                mes_pagamento = mes + p_idx + 1
                if mes_pagamento < self.num_meses:
                    self.duplicatas_pagar[p_idx][mes_pagamento] += valor_parcela

        self.total_pagamento_compras = [
            self.compras_vista[m] + sum(self.duplicatas_pagar[p][m] for p in range(n_parcelas_compras)) + self.fornecedores_anteriores[m]
            for m in range(self.num_meses)
        ]
    ...
    def gerar_resultados(self):
        ...
        # Agregando totais para a tabela
        ...
        def format_row(label, values):
            ...
            return [label] + [f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") for v in values] + [f"R$ {total:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")]
        ...
        resultados_ordenados = [
            ...
            format_row('(-) Fornecedores a pagar anteriores', self.fornecedores_anteriores), # NOVO CAMPO
        ...
        indicadores = {
            'Total de Vendas': f"R$ {sum(self.previsao_vendas):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            'Total de Recebimentos': f"R$ {total_recebimentos:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            'Total de Despesas': f"R$ {sum(self.total_comissoes) + sum(self.total_pagamento_compras) + sum(self.desp_variaveis) + sum(self.desp_fixas):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            'Saldo Final Acumulado': f"R$ {self.saldo_final_caixa[-1]:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
...
# --- Rotas da Aplicação ---
# (As rotas de autenticação e outras rotas auxiliares permanecem as mesmas)
...
def index():
    ...
    if not user.has_active_subscription(): return redirect(url_for('payment'))
...
def calcular_route():
    ...
    if not user or not user.has_active_subscription():
        return jsonify({'error': 'Assinatura inválida', 'redirect_url': url_for('payment')}), 403
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=True) # Debug True para desenvolvimento local
