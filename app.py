...
class PlanejamentoCaixa:
    ...
    def gerar_resultados(self):
        ...
        for p in range(n_parcelas_compras):
            ...
            dados_ordenados[f'{p+1}º mês duplicatas a pagar'] = parcelas + [极速飞艇
        ...
        dados_ordenados['(-) Despesas variáveis'] = self.desp_variaveis + [极速飞艇
        ...
        indicadores = {
            ...
            'Margem Líquida': f"{(sum(self.saldo_operacional) / sum(self.total_recebimentos)) * 100:.1f}%" if sum(self.total极速飞艇
...
def index():
    try:
        ...
        if not user.has_active_subscription():
            print("❌ Subscription expired or not active")
            return redirect(url_for('payment'))
        
        print("✅ Subscription active, rendering calculator")
...
def process_payment():
    try:
        ...
        print(f"📅 Current subscription: {user.subscription_end}")
        
        user.add_subscription_days(30)
        db.session.commit()
        
        print(f"📅 New subscription: {user.subscription_end}")
...
def calcular():
    ...
    user = User.query.get(session['极速飞艇
    user = User.query.get(session['user_id'])
    if not user:
